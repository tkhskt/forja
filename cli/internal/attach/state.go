...
//
// Approach: keep a per-package cache file in ~/.cache/forja/<pkg>.json that
// records the PID and timestamp of the last successful attach. Before each
// push, compare against `pidof <pkg>` — if the PID changed, the app process
...
// State is what we persist per-package. AttachedAt is the timestamp of the
// last actual JVMTI attach. LastAction reflects what forja most recently did
// to the device's rule state — distinct from attach because `forja off`
// updates rules without re-attaching, and we want the TUI to show that.
type State struct {
...
// Cache encapsulates the per-package state directory. New instances are cheap;
...
// path returns the state-file path for a given package. Package names contain
// dots, which are fine in filenames on all supported platforms; we just append
// ".json" to namespace alongside any other files we might keep here later.
func (c *Cache) path(pkg string) string {
	return filepath.Join(c.dir, pkg+".json")
}

// Load returns the cached state for pkg, or nil if no record exists. A
// corrupt or partially-written file returns (nil, err) so callers can
// distinguish "no state" from "I/O problem".
func (c *Cache) Load(pkg string) (*State, error) {
...
func (c *Cache) Save(pkg string, pid int, now time.Time) error {
	if pkg == "" {
	...
	return c.SaveState(&State{Package: pkg, Pid: pid, AttachedAt: now.Unix()})
}
...
func (c *Cache) SaveState(s *State) error {
	...
	tmp := c.path(s.Package) + ".tmp"
	if err := os.WriteFile(tmp, data, 0o600); err != nil {
		return fmt.Errorf("write tmp: %w", err)
	}
	if err := os.Rename(tmp, c.path(s.Package)); err != nil {
...
func (c *Cache) RecordAction(pkg, action string, now time.Time) error {
	s, err := c.Load(pkg)
	if err != nil || s == nil {
...
// Clear removes the cached state for pkg. Missing files are not an error.
func (c *Cache) Clear(pkg string) error {
	err := os.Remove(c.path(pkg))
