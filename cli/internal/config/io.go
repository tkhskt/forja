...
// Status is one device's enabled state, keyed by Android applicationId. A rule
// is "on" for an app iff its name appears in that app's Enabled list; an absent
// app key (or an empty Enabled list, which normalization collapses to absent)
// means off. Status is the inner value of DeviceStatuses — the on-disk file is
// keyed by device serial first.
type Status map[string]AppStatus

// DeviceStatuses is the on-disk status.json shape: a top-level map of device
// serial → that device's per-app Status. Keying by serial lets the same app
// carry independent enabled sets on different devices.
//
// Lives in the per-project user cache (NOT under .forja/) and is transient
// runtime state — forja rewrites it freely, and a corrupt/legacy file is
// discarded rather than treated as an error (it repopulates on the next push).
//
// e.g. `{"emulator-5554": {"com.example.app": {"enabled": ["rule-a"]}},
//
//	"RZ8N70ABCDE":   {"com.example.app": {"enabled": ["rule-b"]}}}`.
//
// Reads and writes go through MarshalJSON / UnmarshalJSON so the file can carry
// a top-level "$comment" metadata key warning users that the file is
// forja-managed. The `$` prefix is a JSON Schema-style convention; we borrow
// the name here because it sorts to position 0 in json.MarshalIndent output
// (ASCII `$` < `A` < `_` < `a`), so the warning always lands on line 1.
//
// MarshalJSON also enforces the bloat-control invariant: apps with an empty
// Enabled list, and serials left with no apps, are pruned on write. So the
// file only ever holds (device, app) pairs that currently have ≥1 active rule.
type DeviceStatuses map[string]Status
...
// MarshalJSON emits the serial → Status map alongside a top-level $comment key,
// pruning empty apps and empty serials so no dead weight lingers in the file.
func (d DeviceStatuses) MarshalJSON() ([]byte, error) {
	out := make(map[string]json.RawMessage, len(d)+1)
	cb, err := json.Marshal(statusJSONComment)
	if err != nil {
		return nil, err
	}
	out["$comment"] = cb
	for serial, st := range d {
		pruned := Status{}
		for app, as := range st {
			if len(as.Enabled) == 0 {
				continue
		...
		b, err := json.Marshal(map[string]AppStatus(pruned))
		if err != nil {
			return nil, fmt.Errorf("marshal status[%s]: %w", serial, err)
		}
		out[serial] = b
	}
...
// UnmarshalJSON reads the serial → Status map while ignoring any `$`-prefixed
// metadata key. A value that doesn't decode as a Status (e.g. a legacy flat
// entry from before per-device keying) is skipped rather than failing the
// whole load — status.json is transient and legacy data is intentionally
// dropped.
func (d *DeviceStatuses) UnmarshalJSON(data []byte) error {
	...
	for k, v := range raw {
		...
		st := map[string]AppStatus{}
		if err := json.Unmarshal(v, &st); err != nil {
			continue // legacy/unparseable entry — drop it
		}
		(*d)[k] = st
	}
	return nil
...
// ForDevice returns the mutable Status for a serial, creating (and storing) an
// empty one on first access. Because Status is a map, mutating the returned
// value mutates the entry held by d.
func (d DeviceStatuses) ForDevice(serial string) Status {
	st, ok := d[serial]
	if !ok {
		st = Status{}
		d[serial] = st
...
// DeviceApp identifies one app on one device — the unit of push targeting once
// enabled state is per-device.
type DeviceApp struct {
	Serial string
	App    string
}

// TargetsEnabling returns every (serial, app) pair whose enabled list contains
// name, sorted by serial then app. Used by `rules update`/`rules remove` to
// re-push a changed rule to all devices that had it on.
func (d DeviceStatuses) TargetsEnabling(name string) []DeviceApp {
	out := []DeviceApp{}
	for serial, st := range d {
		for _, app := range st.AppsEnabling(name) {
			out = append(out, DeviceApp{Serial: serial, App: app})
		}
	}
...
// DropRule removes name from every app on every device. Called when a rule is
// deleted from the catalog so no orphan enable state lingers.
func (d DeviceStatuses) DropRule(name string) {
	for _, st := range d {
		st.DropRule(name)
	}
}
...
// LoadStatus reads the status file at path. A missing file returns an empty
// DeviceStatuses (not nil) so callers can mutate without nil checks. A file
// that fails to parse (corrupt, or a legacy pre-per-device layout) is also
// treated as empty rather than an error — status.json is transient machine
// state and repopulates on the next push.
func LoadStatus(path string) (DeviceStatuses, error) {
	d := DeviceStatuses{}
	data, err := os.ReadFile(path)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
...
func SaveStatus(path string, d DeviceStatuses) error {
	if d == nil {
		d = DeviceStatuses{}
	}
